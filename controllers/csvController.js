...
function getStockPricesFromApi(stocks, basDt) {
    ...
    const fixedServiceKey = 'DP%2Ba92zdiSA5P1FCb3DWExEvRWD5do%2Fpg5YnIMz%2BpNbG31wug1xhigDi%2BEaUJjO6%2BKbAD73w2xZdKYeF3eviIQ%3D%3D';
        ...
        return axios.get(apiUrl, { params })
            ...
            .then(parsedData => {
                ...
                if (items && Array.isArray(items) && items[0]?.clpr) {
                    return { name: name.toLowerCase(), price: parseFloat(items[0].clpr[0]) };
                }
                console.warn(`No price data found for stock name: ${name}`);
                return { name: name.toLowerCase(), price: null };
            })
            .catch(error => {
                console.error(`Error fetching price for stock name ${name}:`, error.message);
                return { name: name.toLowerCase(), price: null };
            });
    });
...
// 몬테카를로 시뮬레이션 함수
function monteCarloSimulation(data, stockPrices, numPortfolios = 1000) { // 포트폴리오 수를 기본적으로 1000개로 설정
    const stockNames = data.map(stock => stock['Name'].toLowerCase());
    const annualReturns = stockNames.map(name => (stockPrices[name] ? stockPrices[name] / 100 : 0.1));
    ...
    for (let i = 0; i < numPortfolios; i++) {
        ...
        for (let j = 0; j < stockNames.length; j++) {
            for (let k = 0; k < stockNames.length; k++) {
                portfolioRisk += normalizedWeights[j] * normalizedWeights[k] * dailyVolatilities[j] * dailyVolatilities[k];
            }
        }
    ...
    return simulationResults.map(portfolio => {
        ...
        for (const stockName in portfolio.weights) {
            ...
            if (stockPrice > remainingAmount) {
                console.log(`${stockName}의 가격이 ${remainingAmount}보다 큽니다. 구매 불가.`);
            ...
            // 최소 한 주를 구매할 수 있는 경우
            let shares = Math.max(1, Math.floor((remainingAmount * portfolio.weights[stockName]) / stockPrice));
        ...
        const sortedStocks = Object.keys(portfolio.weights).sort((a, b) => portfolio.weights[b] - portfolio.weights[a]);
        for (const stockName of sortedStocks) {
            ...
            if (additionalShares > 0) {
                ...
                stockPurchaseDetails[stockName].totalCost += additionalShares * stockPrice;
...
const purchaseStocks = async (req, res) => {
    ...
    if (!req.session || !req.session.dominantTrait) {
        return res.status(400).send('Session data is missing. Please set the session first.');
    }
    const count=req.session.count;
    ...
    if (!count || !amount || !basDt) {
        return res.status(400).send('Missing required query parameters: surveyResult, count, amount, basDt');
    }
